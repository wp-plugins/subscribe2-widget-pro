...
Description: Easily display Subscribe2 business ratings with a simple and intuitive WordPress widget.
Version: 1.1
Author: Devin Walker
...
function add_subscribe2_widget_css() {
    ...
    $cssOption = get_option('subscribe2_widget_settings');

    if($cssOption["subscribe2_widget_disable_css"] == 0) {

        $url = WP_PLUGIN_URL . '/subscribe2-widget-pro/style/subscribe2.css';
        $dir = WP_PLUGIN_DIR . '/subscribe2-widget-pro/style/subscribe2.css';

        if (file_exists($dir)) {
            wp_register_style('subscribe2-widget', $url);
            wp_enqueue_style('subscribe2-widget');
        }
    }
...
/**
 * Adds Subscribe2 Widget Pro Widget
 */
