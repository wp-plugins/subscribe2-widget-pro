...
class Subscribe2_Widget_Pro extends WP_Widget {
	...
	public function add_s2w_widget_frontend_scripts() {
		...
		if ( ! isset( $options["s2w_widget_disable_css"] ) || $options["s2w_widget_disable_css"] == 0 ) {
			...
			$url = plugins_url( '/includes/style/s2w-style.css', dirname( __FILE__ ) );
	...
	function form( $instance ) {
		...
		if ( $options == false ) {
			...
			$defaults = array(
				...
				'disable_antispam'        => $options['disable_antispam']
			);
		...
		$title                   = empty( $instance['title'] ) ? '' : htmlspecialchars( $instance['title'], ENT_QUOTES );
		$div                     = empty( $instance['div'] ) ? '' : htmlspecialchars( $instance['div'], ENT_QUOTES );
		$widgetprecontent        = empty( $instance['widgetprecontent'] ) ? '' : htmlspecialchars( $instance['widgetprecontent'], ENT_QUOTES );
		$widgetpostcontent       = empty( $instance['widgetpostcontent'] ) ? '' : htmlspecialchars( $instance['widgetpostcontent'], ENT_QUOTES );
		$hide_subscribe_button   = empty( $instance['hide_subscribe_button'] ) ? '' : htmlspecialchars( $instance['hide_subscribe_button'], ENT_QUOTES );
		$hide_unsubscribe_button = empty( $instance['hide_unsubscribe_button'] ) ? '' : htmlspecialchars( $instance['hide_unsubscribe_button'], ENT_QUOTES );
		$hide_widget_title       = empty( $instance['hide_widget_title'] ) ? '' : htmlspecialchars( $instance['hide_widget_title'], ENT_QUOTES );
		$postto                  = empty( $instance['postto'] ) ? '' : htmlspecialchars( $instance['postto'], ENT_QUOTES );
		$validation_message      = empty( $instance['validation_message'] ) ? '' : htmlspecialchars( $instance['validation_message'], ENT_QUOTES );
		$success_message         = empty( $instance['success_message'] ) ? '' : htmlspecialchars( $instance['success_message'], ENT_QUOTES );
		$unsubscribe_message     = empty( $instance['unsubscribe_message'] ) ? '' : htmlspecialchars( $instance['unsubscribe_message'], ENT_QUOTES );
		$disable_antispam        = empty( $instance['disable_antispam'] ) ? '' : htmlspecialchars( $instance['disable_antispam'], ENT_QUOTES );
